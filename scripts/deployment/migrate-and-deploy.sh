...
async function migrate() {
    ...
    try {
        ...
        console.log('   🔄 Dropping old contact unique index...');
        try {
        ...
        console.log('   🔄 Adding newsletter_consent to existing entries...');
    ...
    try {
        ...
        console.log('   🔄 Adding unrealized_pnl field...');
...
if curl -f -s "http://localhost:$PORT/api/markets" > /dev/null 2>&1; then
    ...
    warning "⚠️  API may not be responding yet, check logs"
fi
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    if echo "$RESPONSE" | grep -q "access_code\|Already registered"; then
        log "✅ Waitlist endpoint working"
    else
        warning "⚠️  Waitlist endpoint may have issues"
    ...
    if [ -n "$MARKETS" ] && [ "$MARKETS" != "null" ]; then
        log "✅ Markets endpoint working"
    else
        warning "⚠️  Markets endpoint may have issues"
    fi
...
echo ""
log "🚀 All done! Your backend is live."
echo ""
